...
//    }
    ...
    cv::Size size(120*3, 80*3);

    cv::resize(Brushfire, BigBrush, size);
    cv::resize(imageOrg, originalScaled, size);
    std::cout << std::endl;

    //cv::threshold(BigBrush, BigBrush, 180, 255, cv::THRESH_BINARY);
...
//        std::cout << "begin " << *contour.begin() << " size " << contour.size() << std::endl;
//        std::cout << *localMax << std::endl;
        cv::circle(originalScaled, *localMax, 2, cv::Scalar(0,255,0));
    }

    for(size_t i = 0; i < contours.size(); i++) {
        cv::drawContours(originalScaled, contours, i, cv::Scalar(0,0,255));
